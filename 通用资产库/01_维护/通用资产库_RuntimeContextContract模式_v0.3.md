...
# 通用资产库｜Runtime Context Contract 模式 v0.3
...
>
> v0.3 在 v0.2 的 Sidecar 规则基础上新增两项 Wave 2 稳定结论：
>
> 1. **Background deterministic progression != Model Activation**：长期 Need / timer / cooldown / routine automation 可以由 Program 后台推进，而不持续调用模型；
> 2. **Package Included != Feature Enabled != Module Enabled != Runtime Relevant != Model Visible**：带 Feature / Module 的大型 Package 必须按实际启用层级裁剪 Router Directory 与 Context。
...
对于带内部 Feature / Module 的 Expansion，进一步展开为：
...
Router Directory 默认只包含当前 Game 真正启用的 Feature / Module Routing Profile；关闭的 Feature / Module 不应因为“Package 理论上支持”而进入 Router Prompt 或产生 Conditional Dependency。

---

# 2. Context Contract 标准段落

每个新 Core / 重要 Expansion 至少回答以下 13 项：
...
带内部 Feature / Module 的 Package 必须回答：

- Package 被当前 Game Include 后，哪些 Feature 真正 Enabled；
- Feature Enabled 后，哪些 Module 真正 Enabled；
- Router Directory 是否只暴露当前启用层级；
- Module Conditional Dependency 是否只在对应 Module 启用 /调用时成立。

禁止：`Package capability breadth = Router / Context breadth`。

## 2.13 Background Program Progression

长期状态若可由 Program deterministic、安全、可测试地推进，例如：
...
通用库维护一份唯一索引，记录每个资产的 current Context Contract 位于 embedded-in-asset 或 sidecar。
...
14. 多 Feature / Module Package 是否把关闭能力错误放进 Router Directory？
15. deterministic background progression 是否被错误实现成持续模型调用？
...
## Wave 2 Background / Feature-Module Retrofit

Survival 验证：长期 Need / Exposure / Routine Automation 可以 Program-side background progression，不持续激活模型。

Traveler/System 验证：Package / Feature / Module activation hierarchy 可以裁剪 Router Directory，Module Conditional Dependency 不等于包级 Context Inclusion。
...
WEB-04 Final Host Convergence 已 PASS / CLOSED；G9 machine contract / compiler / Context Router implementation 仍被 G8 Exit Gate 阻塞。

本 Pattern 允许继续进行 **Semantic Retrofit**，并作为未来 G9-02 handwritten runtime profiles 的输入；但不冻结 Schema 字段、Router API、Context Compiler、Retrieval Algorithm、token budget、模型调用次数或 Provider-specific prompt layout，也不通过资产反向发明 Host Capability。